...
class WiremockClientTest {
    ...
    @BeforeEach
    void setUp() {
        wiremockClient = new WiremockClient(restClient, "http://api.wiremock.com/cep/{cep}");
    ...
    void consultarCep_DeveRetornarCepResponse_QuandoApiRetornaDados() {
        ...
        when(requestHeadersUriSpecMock.uri("http://api.wiremock.com/cep/{cep}", cep)).thenReturn(requestHeadersUriSpecMock);
        ...
        // Act
        CepResponse actualResponse = wiremockClient.consultarCep(cep);
    ...
    void consultarCep_DeveLancarExcecao_QuandoApiFalha() {
        ...
        // Act & Assert
        try {
            wiremockClient.consultarCep(cep);
